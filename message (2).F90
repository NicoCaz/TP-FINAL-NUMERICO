...
PROGRAM TPFinal
	...
	!Esfuerzo 1
		...
		M(1,3)=0														!M(i,3)=Valor esfuerzo i
	!Esfuerzo 2
		M(2,1)=5
		M(2,2)=pi/2.
		M(2,3)=0
...
!####SECCION EJECUTABLE#################################################
	...
	INDICE=1
	DO WHILE (v(0)<=L)
		TRAMO=TRAMO+M(indice,3)*sin(m(indice,2))*(v(0)-m(indice,1))
		INDICE=INDICE+1
		DO WHILE (v(0)<=m(indice,1))		
...
!#######################################################################	
	...
	FUNCTION v_prima(v)
		...
		v_prima(1)=v(0)!y respecto a x	
		v_prima(2)=(TRAMO/(ELASTICA*INERCIA))*((1+(v(2)**2))**(3/2.)) !averiguar si es v_prima o v(2)
